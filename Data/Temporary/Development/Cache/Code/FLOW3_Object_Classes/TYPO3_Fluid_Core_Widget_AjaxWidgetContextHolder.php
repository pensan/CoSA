...
class AjaxWidgetContextHolder extends AjaxWidgetContextHolder_Original implements \TYPO3\FLOW3\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		...
		if (get_class($this) === 'TYPO3\Fluid\Core\Widget\AjaxWidgetContextHolder') \TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->setInstance('TYPO3\Fluid\Core\Widget\AjaxWidgetContextHolder', $this);
	}
	 ...
	 */
	 public function store(\TYPO3\Fluid\Core\Widget\WidgetContext $widgetContext) {
		...
		if (isset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['store'])) {
		$result = parent::store($widgetContext);

		} else {
			$this->FLOW3_AOP_Proxy_methodIsInAdviceMode['store'] = TRUE;
			try {
				...
				$methodArguments['widgetContext'] = $widgetContext;
					...
					$advices = $this->FLOW3_AOP_Proxy_targetMethodsAndGroupedAdvices['store']['TYPO3\FLOW3\AOP\Advice\BeforeAdvice'];
					$joinPoint = new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\Fluid\Core\Widget\AjaxWidgetContextHolder', 'store', $methodArguments);
					foreach ($advices as $advice) {
						$advice->invoke($joinPoint);
					}

					$adviceChains = $this->FLOW3_AOP_Proxy_getAdviceChains('store');
					$adviceChain = $adviceChains['TYPO3\FLOW3\AOP\Advice\AroundAdvice'];
					$adviceChain->rewind();
					$result = $adviceChain->proceed(new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\Fluid\Core\Widget\AjaxWidgetContextHolder', 'store', $methodArguments, $adviceChain));

			} catch(\Exception $e) {
				unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['store']);
				throw $e;
			}
			unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['store']);
		}
	 ...
	 */
	 public function get($ajaxWidgetId) {
		...
		if (isset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['get'])) {
		$result = parent::get($ajaxWidgetId);

		} else {
			$this->FLOW3_AOP_Proxy_methodIsInAdviceMode['get'] = TRUE;
			try {
				...
				$methodArguments['ajaxWidgetId'] = $ajaxWidgetId;
					...
					$adviceChains = $this->FLOW3_AOP_Proxy_getAdviceChains('get');
					$adviceChain = $adviceChains['TYPO3\FLOW3\AOP\Advice\AroundAdvice'];
					$adviceChain->rewind();
					$result = $adviceChain->proceed(new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\Fluid\Core\Widget\AjaxWidgetContextHolder', 'get', $methodArguments, $adviceChain));

			} catch(\Exception $e) {
				unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['get']);
				throw $e;
			}
			unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['get']);
		}
...
#0             %CLASS%TYPO3_Fluid_Core_Widget_AjaxWidgetContextHolder12208     