...
class FlashMessageContainer extends FlashMessageContainer_Original implements \TYPO3\FLOW3\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		...
		if (get_class($this) === 'TYPO3\FLOW3\MVC\FlashMessageContainer') \TYPO3\FLOW3\Core\Bootstrap::$staticObjectManager->setInstance('TYPO3\FLOW3\MVC\FlashMessageContainer', $this);
	}
	 ...
	 */
	 public function addMessage(\TYPO3\FLOW3\Error\Message $message) {
		...
		if (isset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['addMessage'])) {
		$result = parent::addMessage($message);

		} else {
			$this->FLOW3_AOP_Proxy_methodIsInAdviceMode['addMessage'] = TRUE;
			try {
				...
				$methodArguments['message'] = $message;
					...
					$advices = $this->FLOW3_AOP_Proxy_targetMethodsAndGroupedAdvices['addMessage']['TYPO3\FLOW3\AOP\Advice\BeforeAdvice'];
					$joinPoint = new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\FLOW3\MVC\FlashMessageContainer', 'addMessage', $methodArguments);
					foreach ($advices as $advice) {
						$advice->invoke($joinPoint);
					}

					$adviceChains = $this->FLOW3_AOP_Proxy_getAdviceChains('addMessage');
					$adviceChain = $adviceChains['TYPO3\FLOW3\AOP\Advice\AroundAdvice'];
					$adviceChain->rewind();
					$result = $adviceChain->proceed(new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\FLOW3\MVC\FlashMessageContainer', 'addMessage', $methodArguments, $adviceChain));

			} catch(\Exception $e) {
				unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['addMessage']);
				throw $e;
			}
			unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['addMessage']);
		}
	 ...
	 */
	 public function flush($severity = NULL) {
		...
		if (isset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['flush'])) {
		$result = parent::flush($severity);

		} else {
			$this->FLOW3_AOP_Proxy_methodIsInAdviceMode['flush'] = TRUE;
			try {
				...
				$methodArguments['severity'] = $severity;
					...
					$advices = $this->FLOW3_AOP_Proxy_targetMethodsAndGroupedAdvices['flush']['TYPO3\FLOW3\AOP\Advice\BeforeAdvice'];
					$joinPoint = new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\FLOW3\MVC\FlashMessageContainer', 'flush', $methodArguments);
					foreach ($advices as $advice) {
						$advice->invoke($joinPoint);
					}

					$adviceChains = $this->FLOW3_AOP_Proxy_getAdviceChains('flush');
					$adviceChain = $adviceChains['TYPO3\FLOW3\AOP\Advice\AroundAdvice'];
					$adviceChain->rewind();
					$result = $adviceChain->proceed(new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\FLOW3\MVC\FlashMessageContainer', 'flush', $methodArguments, $adviceChain));

			} catch(\Exception $e) {
				unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['flush']);
				throw $e;
			}
			unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['flush']);
		}
	 ...
	 */
	 public function getMessages($severity = NULL) {
		...
		if (isset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['getMessages'])) {
		$result = parent::getMessages($severity);

		} else {
			$this->FLOW3_AOP_Proxy_methodIsInAdviceMode['getMessages'] = TRUE;
			try {
				...
				$methodArguments['severity'] = $severity;
					...
					$adviceChains = $this->FLOW3_AOP_Proxy_getAdviceChains('getMessages');
					$adviceChain = $adviceChains['TYPO3\FLOW3\AOP\Advice\AroundAdvice'];
					$adviceChain->rewind();
					$result = $adviceChain->proceed(new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\FLOW3\MVC\FlashMessageContainer', 'getMessages', $methodArguments, $adviceChain));

			} catch(\Exception $e) {
				unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['getMessages']);
				throw $e;
			}
			unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['getMessages']);
		}
	 ...
	 */
	 public function getMessagesAndFlush($severity = NULL) {
		...
		if (isset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['getMessagesAndFlush'])) {
		$result = parent::getMessagesAndFlush($severity);

		} else {
			$this->FLOW3_AOP_Proxy_methodIsInAdviceMode['getMessagesAndFlush'] = TRUE;
			try {
				...
				$methodArguments['severity'] = $severity;
					...
					$adviceChains = $this->FLOW3_AOP_Proxy_getAdviceChains('getMessagesAndFlush');
					$adviceChain = $adviceChains['TYPO3\FLOW3\AOP\Advice\AroundAdvice'];
					$adviceChain->rewind();
					$result = $adviceChain->proceed(new \TYPO3\FLOW3\AOP\JoinPoint($this, 'TYPO3\FLOW3\MVC\FlashMessageContainer', 'getMessagesAndFlush', $methodArguments, $adviceChain));

			} catch(\Exception $e) {
				unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['getMessagesAndFlush']);
				throw $e;
			}
			unset($this->FLOW3_AOP_Proxy_methodIsInAdviceMode['getMessagesAndFlush']);
		}
	...
	$reflectedClass = new \ReflectionClass('TYPO3\FLOW3\MVC\FlashMessageContainer');
	...
	foreach($allReflectedProperties as $reflectionProperty) {
		...
		if ($reflectionService->isPropertyTaggedWith('TYPO3\FLOW3\MVC\FlashMessageContainer', $propertyName, 'transient')) continue;
...
#0             %CLASS%TYPO3_FLOW3_MVC_FlashMessageContainer15524     